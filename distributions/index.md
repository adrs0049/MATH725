...
$C_c^\infty$ rather than just $C_c$. Write
$\mathcal{D}(\Omega) = C_c^\infty(\Omega)$ for this space of test functions.

Distribution theory resolves all three issues in one stroke: define a
**distribution** to be a continuous linear functional on
$\mathcal{D}(\Omega)$. Every locally integrable
...
justifies identifying a function with the distribution it defines, so we
