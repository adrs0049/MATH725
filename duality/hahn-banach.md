...
on $\overline{M} + \text{span}(x_0)$ by $g(m + \alpha x_0) = \alpha$. Then $|g(m
+ \alpha x_0)| = |\alpha| \leq \|m + \alpha x_0\|/d$ (since $\|m + \alpha x_0\|
\geq |\alpha| d$). So $\|g\| \leq 1/d$. Hahn-Banach extends $g$ to $f \in X^*$.
By construction $f = 0$ on $M$ but $f(x_0) = 1 \neq 0$.
```
````
...
### The hierarchy of separation results

| What we separate | What we need | Result |
|---|---|---|
| Two points $x \neq y$ | Just $x \neq y$ | {prf:ref}`hb-separation` |
| A point from the unit ball | $\|x_0\| > 1$ | {prf:ref}`hb-norming` |
| A point from a closed convex set $C$ | $x_0 \notin C$, $C$ closed and convex | {prf:ref}`geometric-hahn-banach` |
| Two disjoint closed convex sets | One compact, one closed | Strict separation theorem |

Each row is a stronger geometric statement, but they all flow from the same
source: the analytic Hahn–Banach theorem guarantees a norm-preserving extension,
and the extension defines a separating hyperplane.
